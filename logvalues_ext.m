...
if(t<Tstress) % If temperature is ramping up/down or equal to the cooling temperature
    if(toc-MD(MUnb).ExpData.log.Ttime(end)>=Ttimestep || MD(MUnb).ExpData.log.Ttime==0) % if the time elapsed since last log is larger or equal to the user-defined timestep for temperature log, or if the first value is 0.
