...
fopen(IMPA); %Open gpib

%{
%Define CV program to be read in by Impedance Analyzer (parallel C)
Prog_CV = "PROG"...
    + "'10 FNC1'," ... %Impedance Measurement
    + "'20 SWM2'," ... % Single Mode Sweep
    + "'30 IMP11'," ... % Cp-Rp Circuit
...
% %{
%Define CV program to be read in by Impedance Analyzer (series capacitance)
Prog_CV = "PROG"...
    + "'10 FNC1'," ... %Impedance Measurement
    + "'20 SWM2'," ... % Single Mode Sweep
...
for p=1:length(ArdP)
    ...
    subplot(2,4,ArdP(p)-1);
    plot(Vmeas,Cmeas*1e12);
    ylabel('Capacitance (pF)');
    xlabel('Voltage (V)');
    legend_txt="Pin"+num2str(p);
    legend(legend_txt);
    writeDigitalPin(Arduino,char("D"+num2str(ArdP(p))),0); %Turn off desired pogo-pin
end
% title(com);
fclose(IMPA);
clear all
