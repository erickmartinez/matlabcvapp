...
if(time_inc>=biastime_sec && getTC(app,MUnb)<=StressT+3 && getTC(app,MUnb)>= StressT-3) % Allow error of +/- 3 °C in temperature. Temperature condition to avoid turning on the fan if ramping has already started or if room T has been reached
    ...
    % Stop hotplate
    setHotPlateTemperature(app, app.HW(MUnb).HP, MD(mu).ExpData.Setup.CalTempC); % Set hotplate temperature to the cool temperature, corrected using the calibration curve
    setHotPlateTemperature(app, app.HW(MUnb).HP, MD(mu).ExpData.Setup.CalTempC);
