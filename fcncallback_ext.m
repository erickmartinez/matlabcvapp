...
Temp=getTC(app,MUnb);
if(abs(Temp - CoolT) <= Err && MD(MUnb).MDdata.meas_flag==0 ...
        && MD(MUnb).MDdata.stress_completed_flag==1 && MD(MUnb).MDdata.finish_flag ~= 1) % What if both bias and measurement are performed at room temperature? add a stress completed flag
